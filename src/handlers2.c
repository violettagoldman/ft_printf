...
void	ft_hexa(long int i, t_format *format, int *count, char *str, char *m)
{
	int fill_spaces;
	int fill_zero;
...
void	ft_hexa_help(long int i, t_format *format, int *fill_spaces, int *fill_zero)
{
	...
	if (ft_abs(format->width) > len_nb_hexa(i, format) + *fill_zero)
		*fill_spaces = ft_abs(format->width) - len_nb_hexa(i, format) - *fill_zero;
...
{
	format->flags[4] = 1;
	ft_hexa(i, format, count, "0123456789abcdef", "0x");
}
